...
// DepartmentRepository Department数据仓库接口
type DepartmentRepository interface {
	...
	// Find 查询记录列表
	Find(ctx context.Context, filter bson.M) ([]*models.Department, error)

	// FindOne 查询单条记录
	FindOne(ctx context.Context, filter bson.M) (*models.Department, error)

	// FindWithPage 分页查询记录列表
	FindWithPage(ctx context.Context, filter bson.M, page, pageSize int64) ([]*models.Department, error)
	...
	FindDeletedWithPage(ctx context.Context, filter bson.M, page, pageSize int64) ([]*models.Department, error)
	...
	FindByTenant(ctx context.Context, tenantID string, filter bson.M) ([]*models.Department, error)

	// FindByTenantWithPage 分页查询租户下的记录（包括公共数据）
	FindByTenantWithPage(ctx context.Context, tenantID string, filter bson.M, page, pageSize int64) ([]*models.Department, error)
...
// departmentRepository Department数据仓库实现
type departmentRepository struct {
	dbManager *database.DatabaseManager
}

// NewDepartmentRepository 创建Department数据仓库实例
...
func (r *departmentRepository) getCollection(ctx context.Context) *mongo.Collection {
...
func (r *departmentRepository) Get(ctx context.Context, id string) (*models.Department, error) {
...
// Find 查询记录列表
func (r *departmentRepository) Find(ctx context.Context, filter bson.M) ([]*models.Department, error) {
	collection := r.getCollection(ctx)
...
// FindOne 查询单条记录
func (r *departmentRepository) FindOne(ctx context.Context, filter bson.M) (*models.Department, error) {
	collection := r.getCollection(ctx)
	dept := &models.Department{}
	err := collection.FindOne(ctx, filter).Decode(dept)
	if err != nil {
...
func (r *departmentRepository) FindWithPage(ctx context.Context, filter bson.M, page, pageSize int64) ([]*models.Department, error) {
	collection := r.getCollection(ctx)
...
// Count 统计记录数
func (r *departmentRepository) Count(ctx context.Context, filter bson.M) (int64, error) {
	collection := r.getCollection(ctx)
...
func (r *departmentRepository) Create(ctx context.Context, dept *models.Department) error {
	// 验证必填字段
	if err := validateDepartmentData(dept); err != nil {
		return err
	}
	...
	// 将生成的 ObjectID 转换为字符串并设置到 dept.ID
...
// validateDepartmentData 验证部门数据的必填字段
func validateDepartmentData(dept *models.Department) error {
	// 验证 name 不能为空或纯空白
	if dept.Name == "" || strings.TrimSpace(dept.Name) == "" {
		return fmt.Errorf("部门名称不能为空或纯空白字符")
	}

	// 验证 code 不能为空或纯空白
	if dept.Code == "" || strings.TrimSpace(dept.Code) == "" {
		return fmt.Errorf("部门编码不能为空或纯空白字符")
	}
...
// Update 更新记录
func (r *departmentRepository) Update(ctx context.Context, id string, update bson.M) error {
	...
	// 如果更新中包含 code 字段，验证不能为空或纯空白
	if code, exists := update["code"]; exists {
		if codeStr, ok := code.(string); ok {
			if codeStr == "" || strings.TrimSpace(codeStr) == "" {
				return fmt.Errorf("部门编码不能为空或纯空白字符")
			}
		}
	}
...
// UpdateOne 按条件更新单条记录
func (r *departmentRepository) UpdateOne(ctx context.Context, filter bson.M, update bson.M) error {
	// 如果更新中包含 name 字段，验证不能为空或纯空白
	if name, exists := update["name"]; exists {
		if nameStr, ok := name.(string); ok {
			if nameStr == "" || strings.TrimSpace(nameStr) == "" {
...
// DeleteMany 批量删除
func (r *departmentRepository) DeleteMany(ctx context.Context, filter bson.M) (int64, error) {
	collection := r.getCollection(ctx)
...
func (r *departmentRepository) FindDeletedWithPage(ctx context.Context, filter bson.M, page, pageSize int64) ([]*models.Department, error) {
	collection := r.getCollection(ctx)
	// 添加已删除条件
	filter["is_deleted"] = 1
...
// CountDeleted 统计已删除记录数
func (r *departmentRepository) CountDeleted(ctx context.Context, filter bson.M) (int64, error) {
	collection := r.getCollection(ctx)
	filter["is_deleted"] = 1
	count, err := collection.CountDocuments(ctx, filter)
	if err != nil {
...
func (r *departmentRepository) RestoreOne(ctx context.Context, id string) error {
	collection := r.getCollection(ctx)
...
// RestoreMany 批量恢复记录
func (r *departmentRepository) RestoreMany(ctx context.Context, ids []string) (int64, error) {
	collection := r.getCollection(ctx)
	if len(ids) == 0 {
		return 0, nil
	}
...
func (r *departmentRepository) HardDelete(ctx context.Context, id string) error {
	collection := r.getCollection(ctx)
	// 将字符串 ID 转换为 ObjectID 进行删除
	objectID, err := bson.ObjectIDFromHex(id)
	if err != nil {
...
// HardDeleteMany 批量物理删除记录
func (r *departmentRepository) HardDeleteMany(ctx context.Context, ids []string) (int64, error) {
	collection := r.getCollection(ctx)
	if len(ids) == 0 {
		return 0, nil
	}
...
// 已废弃：请使用 GetCollectionWithContext 以支持租户隔离
func (r *departmentRepository) GetCollection() *mongo.Collection {
	return r.dbManager.GetSystemDatabase().Collection("department")
}

// GetCollectionWithContext 根据Context获取MongoDB集合（支持租户隔离）
func (r *departmentRepository) GetCollectionWithContext(ctx context.Context) *mongo.Collection {
	return r.getCollection(ctx)
}

// FindByTenant 查询租户下的记录（包括公共数据）
func (r *departmentRepository) FindByTenant(ctx context.Context, tenantID string, filter bson.M) ([]*models.Department, error) {
...
// FindByTenantWithPage 分页查询租户下的记录（包括公共数据）
func (r *departmentRepository) FindByTenantWithPage(ctx context.Context, tenantID string, filter bson.M, page, pageSize int64) ([]*models.Department, error) {
	// 添加租户条件：自己的数据 OR 公共数据
	filter["$or"] = []bson.M{
		{"tenant_id": tenantID},
		{"is_common": true},
...
// CountByTenant 统计租户下的记录数（包括公共数据）
func (r *departmentRepository) CountByTenant(ctx context.Context, tenantID string, filter bson.M) (int64, error) {
	// 添加租户条件：自己的数据 OR 公共数据
	filter["$or"] = []bson.M{
		{"tenant_id": tenantID},
		{"is_common": true},
...
func (r *departmentRepository) CheckOwnership(ctx context.Context, id string, tenantID string) (bool, error) {
	dept, err := r.Get(ctx, id)
	if err != nil {
		return false, err
	}
	if dept == nil {
