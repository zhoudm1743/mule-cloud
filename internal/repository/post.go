...
// PostRepository Post数据仓库接口
type PostRepository interface {
	// Get 根据ID获取单条记录
	Get(ctx context.Context, id string) (*models.Post, error)

	// GetByName 根据name获取单条记录（按创建时间倒序，返回最新的一条）
	GetByName(ctx context.Context, name string) (*models.Post, error)

	// Find 查询记录列表
	Find(ctx context.Context, filter bson.M) ([]*models.Post, error)

	// FindOne 查询单条记录
	FindOne(ctx context.Context, filter bson.M) (*models.Post, error)

	// FindWithPage 分页查询记录列表
	FindWithPage(ctx context.Context, filter bson.M, page, pageSize int64) ([]*models.Post, error)
	...
	FindDeletedWithPage(ctx context.Context, filter bson.M, page, pageSize int64) ([]*models.Post, error)
	...
	FindByTenant(ctx context.Context, tenantID string, filter bson.M) ([]*models.Post, error)

	// FindByTenantWithPage 分页查询租户下的记录（包括公共数据）
	FindByTenantWithPage(ctx context.Context, tenantID string, filter bson.M, page, pageSize int64) ([]*models.Post, error)
...
// postRepository Post数据仓库实现
type postRepository struct {
	dbManager *database.DatabaseManager
}

// NewPostRepository 创建Post数据仓库实例
...
func (r *postRepository) getCollection(ctx context.Context) *mongo.Collection {
...
func (r *postRepository) GetByName(ctx context.Context, name string) (*models.Post, error) {
	collection := r.getCollection(ctx)
...
// Find 查询记录列表
func (r *postRepository) Find(ctx context.Context, filter bson.M) ([]*models.Post, error) {
	collection := r.getCollection(ctx)
...
func (r *postRepository) FindWithPage(ctx context.Context, filter bson.M, page, pageSize int64) ([]*models.Post, error) {
	collection := r.getCollection(ctx)
...
func (r *postRepository) Create(ctx context.Context, post *models.Post) error {
	...
	// 将生成的 ObjectID 转换为字符串并设置到 post.ID
...
// validatePostData 验证岗位数据的必填字段
func validatePostData(post *models.Post) error {
	// 验证 name 不能为空或纯空白
	if post.Name == "" || strings.TrimSpace(post.Name) == "" {
		return fmt.Errorf("岗位名称不能为空或纯空白字符")
	}

	// 验证 code 不能为空或纯空白
	if post.Code == "" || strings.TrimSpace(post.Code) == "" {
		return fmt.Errorf("岗位编码不能为空或纯空白字符")
	}
...
// Update 更新记录
func (r *postRepository) Update(ctx context.Context, id string, update bson.M) error {
...
// UpdateOne 按条件更新单条记录
func (r *postRepository) UpdateOne(ctx context.Context, filter bson.M, update bson.M) error {
	// 如果更新中包含 name 字段，验证不能为空或纯空白
	if name, exists := update["name"]; exists {
		if nameStr, ok := name.(string); ok {
			if nameStr == "" || strings.TrimSpace(nameStr) == "" {
...
// DeleteMany 批量删除
func (r *postRepository) DeleteMany(ctx context.Context, filter bson.M) (int64, error) {
	collection := r.getCollection(ctx)
...
func (r *postRepository) FindDeletedWithPage(ctx context.Context, filter bson.M, page, pageSize int64) ([]*models.Post, error) {
	collection := r.getCollection(ctx)
	// 添加已删除条件
	filter["is_deleted"] = 1
...
// CountDeleted 统计已删除记录数
func (r *postRepository) CountDeleted(ctx context.Context, filter bson.M) (int64, error) {
	collection := r.getCollection(ctx)
	filter["is_deleted"] = 1
	count, err := collection.CountDocuments(ctx, filter)
	if err != nil {
...
func (r *postRepository) RestoreOne(ctx context.Context, id string) error {
	collection := r.getCollection(ctx)
...
// RestoreMany 批量恢复记录
func (r *postRepository) RestoreMany(ctx context.Context, ids []string) (int64, error) {
	collection := r.getCollection(ctx)
	if len(ids) == 0 {
		return 0, nil
	}
...
func (r *postRepository) HardDelete(ctx context.Context, id string) error {
	collection := r.getCollection(ctx)
	// 将字符串 ID 转换为 ObjectID 进行删除
	objectID, err := bson.ObjectIDFromHex(id)
	if err != nil {
...
// HardDeleteMany 批量物理删除记录
func (r *postRepository) HardDeleteMany(ctx context.Context, ids []string) (int64, error) {
	collection := r.getCollection(ctx)
	if len(ids) == 0 {
		return 0, nil
	}
...
// 已废弃：请使用 GetCollectionWithContext 以支持租户隔离
func (r *postRepository) GetCollection() *mongo.Collection {
	return r.dbManager.GetSystemDatabase().Collection("post")
}

// GetCollectionWithContext 根据Context获取MongoDB集合（支持租户隔离）
func (r *postRepository) GetCollectionWithContext(ctx context.Context) *mongo.Collection {
	return r.getCollection(ctx)
}

// FindByTenant 查询租户下的记录（包括公共数据）
func (r *postRepository) FindByTenant(ctx context.Context, tenantID string, filter bson.M) ([]*models.Post, error) {
...
// FindByTenantWithPage 分页查询租户下的记录（包括公共数据）
func (r *postRepository) FindByTenantWithPage(ctx context.Context, tenantID string, filter bson.M, page, pageSize int64) ([]*models.Post, error) {
	// 添加租户条件：自己的数据 OR 公共数据
	filter["$or"] = []bson.M{
		{"tenant_id": tenantID},
		{"is_common": true},
...
// CountByTenant 统计租户下的记录数（包括公共数据）
func (r *postRepository) CountByTenant(ctx context.Context, tenantID string, filter bson.M) (int64, error) {
	// 添加租户条件：自己的数据 OR 公共数据
	filter["$or"] = []bson.M{
		{"tenant_id": tenantID},
		{"is_common": true},
...
func (r *postRepository) CheckOwnership(ctx context.Context, id string, tenantID string) (bool, error) {
