...
func UpdateRoleHandler(roleSvc *services.RoleService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// 租户权限检查：先查询目标角色
		perm := NewPermissionChecker(c)
		targetRole, err := roleSvc.GetByID(c.Request.Context(), id)
		if err != nil {
			response.Error(c, "角色不存在")
			return
		}
		if err := perm.CheckTenantPermission(targetRole.TenantID, "修改"); err != nil {
			response.Error(c, err.Error())
			return
		}
...
func DeleteRoleHandler(roleSvc *services.RoleService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := perm.CheckTenantPermission(targetRole.TenantID, "删除"); err != nil {
			response.Error(c, err.Error())
			return
		}
