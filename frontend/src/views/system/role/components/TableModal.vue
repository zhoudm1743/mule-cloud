  ...
  if (type === 'add') {
    ...
    if (data) {
      ...
      if (tenantIdToLoad) {
        ...
        console.log('[分配菜单] 租户菜单列表:', tenantMenus.value)
        console.log('[分配菜单] 是否系统超管:', isSystemAdmin.value)
        console.log('[分配菜单] 用户信息:', authStore.userInfo)
        console.log('[分配菜单] 用户角色:', authStore.userInfo?.role)
        console.log('[分配菜单] 用户租户ID:', authStore.userInfo?.tenant_id)
      }
