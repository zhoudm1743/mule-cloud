...
import { createOrderType, updateOrderType } from '@/service'
...
defineOptions({ name: 'OrderTypeModal' })
...
const rules: any = {
  value: { required: true, message: '请输入订单类型名称', trigger: 'blur' },
}
...
async function handleSubmit() {
  ...
  try {
    if (modalType.value === 'add') {
      await createOrderType(formModel as Api.Basic.CreateRequest)
      ...
      await updateOrderType(formModel.id, updateData)
  ...
  <NModal v-model:show="visible" preset="card" :title="title" class="w-600px">
    ...
    >
      <NGrid :cols="1" :x-gap="18">
        <NFormItemGridItem path="value" label="订单类型名称">
          <NInput v-model:value="formModel.value" :disabled="modalType === 'view'" placeholder="请输入订单类型名称" />
        </NFormItemGridItem>
