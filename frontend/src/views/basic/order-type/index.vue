...
import { deleteOrderType, fetchOrderTypeList } from '@/service'
...
async function fetchData() {
  startLoading()
  try {
    const res = await fetchOrderTypeList({
      page: page.value,
      page_size: pageSize.value,
      value: searchForm.value.value || undefined,
    })
    if (res.data) {
      tableData.value = res.data.order_types || []
      total.value = res.data.total || 0
    }
  }
  catch (error: any) {
    window.$message.error(error.message || '获取订单类型列表失败')
  }
  finally {
    endLoading()
  }
...
async function batchDelete() {
  if (checkedRowKeys.value.length === 0) {
    window.$message.warning('请选择要删除的订单类型')
    return
  }
  try {
...
<template>
  <NSpace vertical size="large">
    <NCard title="订单类型管理" :bordered="false" class="rounded-8px shadow-sm">
