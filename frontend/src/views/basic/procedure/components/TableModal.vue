...
import { createProcedure, updateProcedure } from '@/service'
...
defineOptions({ name: 'ProcedureModal' })
...
const rules: any = {
  value: { required: true, message: '请输入工序名称', trigger: 'blur' },
}
...
async function handleSubmit() {
  ...
  try {
    if (modalType.value === 'add') {
      await createProcedure(formModel as Api.Basic.CreateRequest)
      ...
      await updateProcedure(formModel.id, updateData)
  ...
  <NModal v-model:show="visible" preset="card" :title="title" class="w-600px">
    ...
    >
      <NGrid :cols="1" :x-gap="18">
        <NFormItemGridItem path="value" label="工序名称">
          <NInput v-model:value="formModel.value" :disabled="modalType === 'view'" placeholder="请输入工序名称" />
        </NFormItemGridItem>
