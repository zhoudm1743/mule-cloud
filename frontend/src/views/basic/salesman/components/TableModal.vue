...
import { createSalesman, updateSalesman } from '@/service'
...
defineOptions({ name: 'SalesmanModal' })
...
const rules: any = {
  value: { required: true, message: '请输入业务员名称', trigger: 'blur' },
}
...
async function handleSubmit() {
  ...
  try {
    if (modalType.value === 'add') {
      await createSalesman(formModel as Api.Basic.CreateRequest)
      ...
      await updateSalesman(formModel.id, updateData)
  ...
  <NModal v-model:show="visible" preset="card" :title="title" class="w-600px">
    ...
    >
      <NGrid :cols="1" :x-gap="18">
        <NFormItemGridItem path="value" label="业务员名称">
          <NInput v-model:value="formModel.value" :disabled="modalType === 'view'" placeholder="请输入业务员名称" />
        </NFormItemGridItem>
