...
import { deleteSalesman, fetchSalesmanList } from '@/service'
...
async function fetchData() {
  startLoading()
  try {
    const res = await fetchSalesmanList({
      page: page.value,
      page_size: pageSize.value,
      value: searchForm.value.value || undefined,
    })
    if (res.data) {
      tableData.value = res.data.salesmans || []
      total.value = res.data.total || 0
    }
  }
  catch (error: any) {
    window.$message.error(error.message || '获取业务员列表失败')
  }
  finally {
    endLoading()
  }
...
async function batchDelete() {
  if (checkedRowKeys.value.length === 0) {
    window.$message.warning('请选择要删除的业务员')
    return
  }
  try {
...
<template>
  <NSpace vertical size="large">
    <NCard title="业务员管理" :bordered="false" class="rounded-8px shadow-sm">
      <n-form :model="searchForm" label-placement="left" inline :show-feedback="false">
        <n-flex>
          <n-form-item label="业务员名称">
            <n-input
