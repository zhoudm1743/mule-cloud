...
import { deleteColor, fetchColorList } from '@/service'
...
async function fetchData() {
  startLoading()
  try {
    const res = await fetchColorList({
      page: page.value,
      page_size: pageSize.value,
      value: searchForm.value.value || undefined,
    })
    if (res.data) {
      tableData.value = res.data.colors || []
      total.value = res.data.total || 0
    }
  }
  catch (error: any) {
    window.$message.error(error.message || '获取颜色列表失败')
  }
  finally {
    endLoading()
  }
...
async function batchDelete() {
  if (checkedRowKeys.value.length === 0) {
    window.$message.warning('请选择要删除的颜色')
    return
  }
  try {
...
<template>
  <NSpace vertical size="large">
    <NCard title="颜色管理" :bordered="false" class="rounded-8px shadow-sm">
