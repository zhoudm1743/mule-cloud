...
import { createColor, updateColor } from '@/service'
...
const rules: any = {
  value: { required: true, message: '请输入颜色名称', trigger: 'blur' },
}
...
async function handleSubmit() {
  ...
  try {
    if (modalType.value === 'add') {
      await createColor(formModel as Api.Basic.CreateRequest)
      ...
      await updateColor(formModel.id, updateData)
  ...
  <NModal v-model:show="visible" preset="card" :title="title" class="w-600px">
    ...
    >
      <NGrid :cols="1" :x-gap="18">
        <NFormItemGridItem path="value" label="颜色名称">
          <NInput v-model:value="formModel.value" :disabled="modalType === 'view'" placeholder="请输入颜色名称" />
        </NFormItemGridItem>
