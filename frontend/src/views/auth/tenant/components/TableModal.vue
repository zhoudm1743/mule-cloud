...
const rules: any = {
  ...
  admin_phone: { 
    ...
    validator: (rule: any, value: string) => {
      if (modalType.value === 'add' && !value) {
        return new Error('新建租户时必须提供管理员手机号')
      }
      return true
    }
  },
  admin_password: { 
    ...
    validator: (rule: any, value: string) => {
      if (modalType.value === 'add' && !value) {
        return new Error('新建租户时必须提供管理员密码')
      }
      if (modalType.value === 'add' && value && value.length < 6) {
    ...
    >
      ...
      <NGrid v-else :cols="2" :x-gap="18">
        <NFormItemGridItem :span="2" path="code" label="租户代码">
          <NInput v-model:value="formModel.code" :disabled="modalType === 'view'" placeholder="请输入租户代码" />
        </NFormItemGridItem>
        <NFormItemGridItem :span="2" path="name" label="租户名称">
          <NInput v-model:value="formModel.name" :disabled="modalType === 'view'" placeholder="请输入租户名称" />
        </NFormItemGridItem>
        <NFormItemGridItem :span="2" path="contact" label="联系人">
          <NInput v-model:value="formModel.contact" :disabled="modalType === 'view'" placeholder="请输入联系人" />
        </NFormItemGridItem>
        <NFormItemGridItem :span="2" path="phone" label="联系电话">
          <NInput v-model:value="formModel.phone" :disabled="modalType === 'view'" placeholder="请输入联系电话" />
        </NFormItemGridItem>
        <NFormItemGridItem :span="2" path="email" label="联系邮箱">
          <NInput v-model:value="formModel.email" :disabled="modalType === 'view'" placeholder="请输入联系邮箱" />
        </NFormItemGridItem>
        ...
        <template v-if="modalType === 'add'">
          <NFormItemGridItem :span="2">
            <n-divider>租户管理员信息</n-divider>
          </NFormItemGridItem>
          <NFormItemGridItem :span="2" path="admin_phone" label="管理员手机号">
            <NInput v-model:value="formModel.admin_phone" placeholder="请输入管理员手机号" />
          </NFormItemGridItem>
          <NFormItemGridItem :span="2" path="admin_password" label="管理员密码">
            <NInput v-model:value="formModel.admin_password" type="password" show-password-on="click" placeholder="请输入管理员密码（至少6位）" />
          </NFormItemGridItem>
          <NFormItemGridItem :span="2" path="admin_name" label="管理员名称">
            <NInput v-model:value="formModel.admin_name" placeholder="请输入管理员名称（可选）" />
          </NFormItemGridItem>
        </template>
      </NGrid>
    </NForm>
