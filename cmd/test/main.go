...
func main() {
	// 解析命令行参数
	configPath := flag.String("config", "config/test.yaml", "配置文件路径")
	flag.Parse()
	...
	loggerPkg.Info("🚀 TestService 启动中...",
	...
	admin := r.Group("/admin")
	{
		admin.GET("/:id", transport.AdminGetHandler(adminSvc))
		admin.DELETE("/:id", transport.AdminDeleteHandler(adminSvc))
		admin.POST("", transport.AdminCreateHandler(adminSvc))
		admin.PUT("/:id", transport.AdminUpdateHandler(adminSvc))
	}
