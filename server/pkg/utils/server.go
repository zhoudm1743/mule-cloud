...
func (su serverUtil) GetCpuInfo() (data map[string]interface{}) {
	...
	if err != nil {
		log.Logger.Errorf("GetCpuInfo Counts err: err=[%+v]", err)
		return data
	}
	tss, err := cpu.Times(false)
	if err != nil {
		log.Logger.Errorf("GetCpuInfo Times err: err=[%+v]", err)
		return data
	}
	ts := tss[0]
...
func (su serverUtil) GetMemInfo() (data map[string]interface{}) {
	...
	if err != nil {
		log.Logger.Errorf("GetMemInfo VirtualMemory err: err=[%+v]", err)
		return data
	}
	return map[string]interface{}{
...
func (su serverUtil) GetSysInfo() (data map[string]interface{}) {
	...
	if err != nil {
		log.Logger.Errorf("GetSysInfo Info err: err=[%+v]", err)
		return data
	}
	pwd, err := os.Getwd()
	if err != nil {
		log.Logger.Errorf("GetSysInfo Getwd err: err=[%+v]", err)
		return data
	}
	return map[string]interface{}{
...
func (su serverUtil) GetDiskInfo() (data []map[string]interface{}) {
	...
	if err != nil {
		log.Logger.Errorf("GetDiskInfo Partitions err: err=[%+v]", err)
		return data
	}
	for i := 0; i < len(partStats); i++ {
		...
		if uErr != nil {
			log.Logger.Errorf("GetDiskInfo Usage err: err=[%+v]", err)
			continue
		}
...
func (su serverUtil) GetGoInfo() (data map[string]interface{}) {
	...
	if err != nil {
		log.Logger.Errorf("GetGoInfo NewProcess err: err=[%+v]", err)
		return data
	}
	memInfo, err := curProc.MemoryInfo()
	if err != nil {
		log.Logger.Errorf("GetGoInfo MemoryInfo err: err=[%+v]", err)
		return data
	}
	startTime, err := curProc.CreateTime()
	if err != nil {
		log.Logger.Errorf("GetGoInfo CreateTime err: err=[%+v]", err)
		return data
	}
	return map[string]interface{}{
		"name":      "Go",
		...
		"startTime": time.UnixMilli(startTime).Format(time.DateTime),
	}
}
